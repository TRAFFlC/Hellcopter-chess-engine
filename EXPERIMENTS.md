...
### 20260831_upgrade_plan_and_upstream_merge

改动: 无引擎代码改动。与上游 origin/main 对齐合并（commit df2abed，评估减法/
时间管理 5 连发/SEE 修复/KRK-KQK 并入）；清理 76 个临时产物；研究报告与迭代
笔记入库；撰写升级规划 docs/UPGRADE_PLAN.md
目的: 用户决策——尽快同步版本管理（远程分叉 23/14，已合拢）；并规划
Hellcopter 提升路线（基于五份研究报告 + 消融教训）

参考来源: 本会话五份研究报告（Pepito/Goldfish/Invictus/Facon/Reckless,
docs/reports/20260831_*.md），报告 §0 各自含反剽窃声明；合并时与上游
不一致的消融判定（C/G/I 模块）以本地 20260719_merge_delete_cgi_failed
交互效应教训为准（上游另议"可删"，本地验证批量删除退化）

核心结论:
1. 合并产物需先 T0/T1 复检（上游 Week7 判定为"进行中"，未过整体确认）——
   Phase 0：合并 vs 合并前基线 300 局快筛，退化则回滚 c116b78 分测
2. 升级分水岭（Facon §6）: 2000→2250 TT 工程+静态分复用 / 2250→2500
   参数置信度 / 2500→2800 全量 Texel / 2800→3100 材料和棋+评估缓存
3. 全程不新增评估特征或搜索技术——参数置信度与执行成本是全部工作
4. 变体兼容（Reckless 结论）: M1-M3 阶段挂起，chess324 门槛低，
   PST 王桶条件化仅当变体入目标再做
5. 红线: 禁基于单模块消融批量删除；一次只改一组参数；借鉴留痕

判定: 完成（规划已存档 docs/UPGRADE_PLAN.md）
理由: 用户指定的同步 + gitignore + 清理 + 规划四项全部落实；下一步执行
规划 Phase 0（编译确认 + 合并产物 T0/T1 复检）
